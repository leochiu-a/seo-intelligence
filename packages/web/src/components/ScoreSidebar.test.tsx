...
describe("ScoreSidebar hierarchy rendering", () => {
  ...
  it("child buttons have deeper left padding vs root", () => {
    ...
    const blogBtn = buttons.find(
      (b) => b.textContent?.includes("/blog") && !b.textContent?.includes("/blog/category"),
    )!;
  ...
  it("clicking a child node calls setNodes and fitView with correct id", async () => {
    ...
    const catBtn = screen
      .getAllByRole("button")
      .find((b) => b.textContent?.includes("/blog/category"))!;
    fireEvent.click(catBtn);
    expect(mockSetNodes).toHaveBeenCalled();
    // fitView is called in a setTimeout — advance timers
    await vi.waitFor(() =>
      expect(mockFitView).toHaveBeenCalledWith(expect.objectContaining({ nodes: [{ id: "n2" }] })),
    );
  });
